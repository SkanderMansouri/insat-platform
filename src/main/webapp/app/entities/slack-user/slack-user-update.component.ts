...
export class SlackUserUpdateComponent implements OnInit {
    ...
    save() {
        ...
        if (this.slackUser.id !== undefined) {
            this.subscribeToSaveResponse(this.slackUserService.update(this.slackUser));
        } else {
            this.subscribeToSaveResponse(this.slackUserService.create(this.slackUser));
        }
    }

    private subscribeToSaveResponse(result: Observable<HttpResponse<ISlackUser>>) {
        result.subscribe((res: HttpResponse<ISlackUser>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
