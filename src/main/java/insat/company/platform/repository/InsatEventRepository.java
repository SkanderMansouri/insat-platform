...
public interface InsatEventRepository extends JpaRepository<InsatEvent, Long> {
    ...
    @Query(value = "select distinct insat_event from InsatEvent insat_event left join fetch insat_event.clubs left join fetch insat_event.members left join fetch insat_event.participants",
    ...
    @Query("select insat_event from InsatEvent insat_event left join fetch insat_event.clubs left join fetch insat_event.members left join fetch insat_event.participants where insat_event.id =:id")
