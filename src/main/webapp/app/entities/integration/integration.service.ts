...
export class IntegrationService {
  ...
  create(integration: IIntegration): Observable<EntityResponseType> {
    return this.http.post<IIntegration>(this.resourceUrl, integration, {
      observe: "response"
    });
  }

  update(integration: IIntegration): Observable<EntityResponseType> {
    return this.http.put<IIntegration>(this.resourceUrl, integration, {
      observe: "response"
    });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IIntegration>(`${this.resourceUrl}/${id}`, {
      observe: "response"
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IIntegration[]>(this.resourceUrl, {
      params: options,
      observe: "response"
    });
  }
