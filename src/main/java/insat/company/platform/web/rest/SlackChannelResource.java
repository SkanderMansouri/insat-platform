...
public class SlackChannelResource {
    ...
    /**
     * SEARCH  /_search/slack-channels?query=:query : search for the slackChannel corresponding
     * to the query.
     *
     * @param query the query of the slackChannel search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/slack-channels")
    @Timed
    public ResponseEntity<List<SlackChannel>> searchSlackChannels(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of SlackChannels for query {}", query);
        Page<SlackChannel> page = slackChannelService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/slack-channels");
