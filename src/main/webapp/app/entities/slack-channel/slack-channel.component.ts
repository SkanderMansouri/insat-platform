...
export class SlackChannelComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.slackChannelService
      ...
      .subscribe(
        (res: HttpResponse<ISlackChannel[]>) => this.paginateSlackChannels(res.body, res.headers),
  ...
  registerChangeInSlackChannels() {
    this.eventSubscriber = this.eventManager.subscribe('slackChannelListModification', response => this.reset());
  }
