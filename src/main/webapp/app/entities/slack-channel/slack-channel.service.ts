...
export class SlackChannelService {
  private resourceUrl = SERVER_API_URL + "api/slack-channels";

  constructor(private http: HttpClient) {}

  create(slackChannel: ISlackChannel): Observable<EntityResponseType> {
    return this.http.post<ISlackChannel>(this.resourceUrl, slackChannel, {
      observe: "response"
    });
  }

  update(slackChannel: ISlackChannel): Observable<EntityResponseType> {
    return this.http.put<ISlackChannel>(this.resourceUrl, slackChannel, {
      observe: "response"
    });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ISlackChannel>(`${this.resourceUrl}/${id}`, {
      observe: "response"
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<ISlackChannel[]>(this.resourceUrl, {
      params: options,
      observe: "response"
    });
  }
