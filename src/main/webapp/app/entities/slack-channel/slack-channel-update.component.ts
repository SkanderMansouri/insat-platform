...
export class SlackChannelUpdateComponent implements OnInit {
  ...
  constructor(private slackChannelService: SlackChannelService, private activatedRoute: ActivatedRoute) {}
  ...
  save() {
    ...
    if (this.slackChannel.id !== undefined) {
      this.subscribeToSaveResponse(this.slackChannelService.update(this.slackChannel));
    } else {
      this.subscribeToSaveResponse(this.slackChannelService.create(this.slackChannel));
    }
  }

  private subscribeToSaveResponse(result: Observable<HttpResponse<ISlackChannel>>) {
    result.subscribe((res: HttpResponse<ISlackChannel>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
  }
