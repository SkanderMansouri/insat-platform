...
export class SlackUserDeleteDialogComponent {
    ...
    constructor(private slackUserService: SlackUserService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class SlackUserDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ slackUser }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(SlackUserDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
