...
export class FieldService {
  ...
  create(field: IField): Observable<EntityResponseType> {
    return this.http.post<IField>(this.resourceUrl, field, {
      observe: "response"
    });
  }

  update(field: IField): Observable<EntityResponseType> {
    return this.http.put<IField>(this.resourceUrl, field, {
      observe: "response"
    });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IField>(`${this.resourceUrl}/${id}`, {
      observe: "response"
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IField[]>(this.resourceUrl, {
      params: options,
      observe: "response"
    });
  }
